...
# ===================================================================
# Fitting (4.167" x 7") • Formula labels • Legend • Label thinning • Paginated
# ===================================================================
library(tidyverse)
...
PANELS_PER_PAGE <- 3     # 1 row x 3 columns per page
NCOL <- 3
FIG_W <- 7
FIG_H <- 4.167
...
for (pg in seq_len(pages)) {
  ...
  pgrid <- wrap_plots(plots[idx_start:idx_end], ncol = NCOL, byrow = TRUE, guides = "collect") +
    plot_annotation(
      ...
      subtitle = "Panels = DOC × pH × context • Focus elements labeled (Fe, Mn, As, Cr, U, Pu, Tc, Se, S, Sb, V)",
      theme = theme(
  ...
  ggsave(sprintf("reaction_network_fitting_SMALL_page%02d.svg", pg), pgrid, width = FIG_W, height = FIG_H)
  ggsave(sprintf("reaction_network_fitting_SMALL_page%02d.png", pg), pgrid, width = FIG_W, height = FIG_H, dpi = 600)
}
