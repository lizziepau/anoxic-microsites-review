...
# ===================================================================
# S-EDGE OVERLAY DEBUG: makes sulfur reaction arrows unmissable
# - Robust ypos (no joins)
# - Element-based node filtering + sulfur/name normalization + key remap
# - Base edges drawn as before, then S-edges overlaid thicker/darker
# - Edge labels shown above arrows; S-edge labels slightly larger
# - Prints S-edge counts per panel to console
# ===================================================================
suppressPackageStartupMessages({
  library(tidyverse); library(ggraph); library(igraph);
  library(patchwork); library(ggrepel); library(stringr)
})
...
sulfur_pat <- "(?i)\\b(SO4|SO₄|HS-|H2S|H₂S|S\\(0\\)|S0|FeS|pyrite|mackinawite|sulfide|sulfate|sulfur)\\b"
nodes$element <- ifelse(
  (is.na(nodes$element) | nodes$element == "" | nodes$element == "default") & str_detect(nodes$name, sulfur_pat),
  "S", nodes$element
)
...
norm_key <- function(x) {
  x %>% str_replace_all("₀","0") %>% str_replace_all("₁","1") %>% str_replace_all("₂","2") %>%
    ...
    str_replace_all("₉","9") %>% str_replace_all("²","^2") %>% str_replace_all("³","^3") %>%
    str_replace_all("−","-") %>% str_replace_all("–","-") %>% str_replace_all("—","-") %>%
    str_replace_all("’","'") %>% str_replace_all("SO₄","SO4") %>% str_replace_all("H₂S","H2S") %>%
...
nodes <- nodes %>% mutate(lane = ifelse(phase_norm %in% names(lane_map), lane_map[phase_norm], 0.50)) %>%
  mutate(lane = case_when(name=="Bulk oxic soil" ~ 0.02, name=="Anoxic microsite" ~ 0.98, TRUE ~ lane))
...
simplify_formula <- function(s) {
  ...
  repl <- list(
    ...
    "FeSe \\(s\\)"="FeSe","V\\(V\\) \\(vanadate\\) \\(aq/sorbed\\)"="V(V)","V\\(IV/III\\) \\(adsorbed/oxide\\)"="V(IV/III)",
    "Organic ligands \\(OM\\)"="OM","Sulfide \\(S2−/HS−\\)"="S2-/HS-",
    "(?i)Sulfate"="SO4^2-","(?i)Sulfide"="S^2-","(?i)Elemental sulfur|S\\(0\\)|S0"="S(0)"
  )
...
stroke_map <- c(Fe="#56B4E9", Mn="#E69F00", As="#009E73", Sb="#CC79A7", Cr="#0072B2",
                Tc="#D55E00", Pu="#F0E442", U="#999999", S="#0099CC", Se="#33CC33", V="#AA4499", default="#666666")
...
# Draw panel with S-edge overlay
draw_panel <- function(sub_edges, title_stub) {
  # Mark S edges in the edge data BEFORE making the graph
  el_by_name <- nodes %>% select(name, element)
  sub_edges2 <- sub_edges %>%
    left_join(el_by_name, by=c("from"="name")) %>% rename(from_el = element) %>%
    left_join(el_by_name, by=c("to"="name"))   %>% rename(to_el   = element) %>%
    mutate(is_S = from_el == "S" | to_el == "S")

  # Quick console counts
  cat(sprintf("[DEBUG] %s — S-edge count: %d / %d\n", title_stub, sum(sub_edges2$is_S, na.rm=TRUE), nrow(sub_edges2)))

  g <- igraph::graph_from_data_frame(d = sub_edges2, vertices = nodes, directed = TRUE)
  ...
  ggraph::ggraph(lay) +
    ...
    # S-edges overlay (thicker/darker, drawn on top)
    ggraph::geom_edge_link(aes(filter = ..index.. %in% which(E(g)$is_S)),
                           color = "black", linewidth = 0.9, alpha = 0.9,
                           arrow = grid::arrow(length = unit(1.6, "mm")),
                           end_cap = ggraph::circle(1.6, 'mm'), show.legend = FALSE) +
    ...
    # Edge labels (S labels slightly larger)
    geom_text(data = e_map, aes(x = xm, y = ym, label = edge_lab),
              size = EDGE_LABEL_SIZE, vjust = 0, lineheight = 0.9) +
    geom_text(data = e_map %>% filter(is_S),
              aes(x = xm, y = ym + 0.12, label = edge_lab),
              size = EDGE_LABEL_SIZE + 0.2, vjust = 0, lineheight = 0.9, fontface = "bold") +
    ...
    scale_y_continuous(breaks = unique(nodes$ypos),
                       labels = as.character(nodes$element)[match(unique(nodes$ypos), nodes$ypos)],
...
make_page <- function(edges_doc_ctx, ctx_label, doc_label) {
  ...
  row + patchwork::plot_annotation(
      title = "Reaction network in oxic soil with anoxic microsites — S-edge overlay debug",
      subtitle = paste0(gsub(" \\(.*?\\)", "", doc_label),
                        " • Columns = pH bands • S edges drawn thicker/darker and labeled"),
...
for (doc in levels(edges$DOC)) {
  for (ctx in levels(edges$context)) {
    ...
    ggsave(sprintf("reaction_network_SDEBUG_%s_%s.svg", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H)
    ggsave(sprintf("reaction_network_SDEBUG_%s_%s.png", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H, dpi = 600)
  }
}
