...
source("make_reaction_network_fig_DOC_legible_FORMULA_LEGEND.R")   # one big plate
source("make_reaction_network_fig_DOC_fitting_FORMULA_LEGEND.R")   # multiple pages

# ===================================================================
# Legible (FORMULA + LEGEND + label thinning, guides collected)
# ===================================================================
library(tidyverse)
...
FIG_W <- 16
FIG_H <- 12
ELEMENTS_TO_SHOW <- c("Fe","Mn","As","Cr","U","Se")  # focus elements for labels
...
draw_panel <- function(sub_edges, panel_title) {
  ...
  ggraph::ggraph(lay) +
    ...
    ggraph::geom_node_point(aes(shape = class, fill = phase, color = element),
                            size = 3.2, stroke = 1, show.legend = TRUE) +
...
ggsave("reaction_network_redox_DOC_legible_FORMULA_LEGEND.svg", plate, width = FIG_W, height = FIG_H)
ggsave("reaction_network_redox_DOC_legible_FORMULA_LEGEND.png", plate, width = FIG_W, height = FIG_H, dpi = 600)
