...
# ===================================================================
# SIX PAGES (DOC × Context) — LEGENDS REMOVED + SEPARATE LEGEND FIGURE
# Phase normalization + numeric pH titles + PROCESS legend (separate)
# X-axis removed
# Output size (pages): 7.0" × 4.167"; Legend: 7.0" × 1.2"
# ===================================================================
...
# ---- Map mediator to three process classes for line type legend ----
edges <- edges %>% mutate(
...
# ---- Build a DOC×Context page (1×3 pH columns, no legends) ----
  ...
  row +
    patchwork::plot_annotation(
      ...
      subtitle = paste0(gsub(" \\(.*?\\)", "", doc_label),
                        " • Columns = pH bands with numeric ranges • Legends exported separately"),
      theme = theme(
...
# ---- Emit SIX pages: DOC × context (no legends) ----
for (doc in levels(edges$DOC)) {
  for (ctx in levels(edges$context)) {
    ...
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_NOLEG_%s_%s.svg", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H)
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_NOLEG_%s_%s.png", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H, dpi = 600)
  }
}

# ===================================================================
# SEPARATE LEGEND FIGURE (Effect on mobility, Process type, Mobility state)
# ===================================================================

# palettes (same as pages)
col_mob <- c(mobilize = "#3b5b92", immobilize = "#a35e10", neutral = "#7a7a7a")
# process linetypes
proc_types <- c(solid = 1, dashed = 2, dotted = 3)
shape_map <- c("Mobile (aq)" = 21, "Immobile/Sorbed" = 22, "Other" = 25)

# Dummy data to generate legends only
df_edges <- expand.grid(
...
df_nodes <- tibble::tibble(
  ...
  x = 0, y = seq_along(mobility_state)
)
...
legend_plot <- ggplot() +
  ...
  geom_point(data = df_nodes,
             aes(x=x, y=y, shape = mobility_state),
