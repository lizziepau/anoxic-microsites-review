...
# ===================================================================
# THREE PANELS (no legend) + SEPARATE LEGEND PANEL (robust, side-by-side)
# Guarantees non-blank legends using synthetic data; extracts via get_legend()
# File: make_concept_heatmap_network_S_VERTICAL_byDOC_BLACKTEXT_NOLEG_AND_LEGEND.R
# ===================================================================
...
# ---- Figure size ----FIG_W <- 7; FIG_H <- 4.167   # per DOC panel
LEG_W <- 6; LEG_H <- 1.85    # legend panel (wide + short)
...
# ---- Palettes ----
#FILL_LOW  <- "#E2A3A1"; FILL_MID  <- "#F2ECE9"; FILL_HIGH <- "#6E5C8E"
#proc_col  <- c(Abiotic="#9DB7C6", `Mineral-mediated`="#8C4A3C", Microbial="#89A15C")

# ---- Palettes ----
#FILL_LOW  <- "#F07E2C"; FILL_MID  <- "#F6F3EE"; FILL_HIGH <- "#62135E"
#proc_col  <- c(Abiotic="#BB3754", `Mineral-mediated`="#FBEC7A", Microbial="#0B0B26")

# ---- Palettes ----
FILL_LOW  <- "#D78DA2"; FILL_MID  <- "#F3F1EE"; FILL_HIGH <- "#6FA5B7"
proc_col  <- c(Abiotic="#8967B0", `Mineral-mediated`="#F0E442", Microbial="#9E3A3A")
...
# ---- Core plot builder (no legend by default) ----
build_plot <- function(df, show_legend = FALSE) {
  ggplot(df, aes(x = pH_band, y = element)) +
    ...
    theme(
      ...
      plot.margin = margin(12,6,4,6)  # space for subheading
    ) +
    labs(title = NULL)
}

# ---- Draw per-DOC page with bold subheading ----
...
# ---- Legend-only panel (side-by-side), using synthetic data to guarantee content ----
save_legend_panel <- function(out_stub = "concept_heatmap_network_LEGEND_PANEL_BLACKTEXT") {
  # Synthetic Process legend data (ensures all categories appear)
  proc_levels <- names(proc_col)
  df_proc <- tibble(
    x = seq_along(proc_levels),
    y = 1,
    dom = factor(proc_levels, levels = proc_levels)
  )
  # Process legend (colored filled circles with black outline)
  p_proc <- ggplot(df_proc, aes(x, y)) +
    # Black outline layer
    geom_point(shape = 21, fill = NA, color = "black", size = 3.6) +
    # Colored fill layer
    geom_point(aes(fill = dom), shape = 21, color = "black", size = 3) +
    scale_fill_manual(values = proc_col, name = "Process") +
    guides(fill = guide_legend(override.aes = list(shape = 21, size = 4, color = "black"))) +
    theme_void(base_size = 10) +
    theme(legend.position = "right")
  leg_proc <- cowplot::get_legend(p_proc)

  # Synthetic Mobility legend data (ensures continuous range present)
  df_fill <- tibble(
    x = 1,
    y = seq_len(50),
  ...
  leg_plot <- cowplot::plot_grid(leg_proc, leg_fill, nrow = 2, rel_heights = c(1, 0.6), align = "v")
...
# ---- Build outputs ----
make_page(doc_labels[1], "concept_heatmap_network_byDOC_LOW_BLACKTEXT_SUB_NOLEG")
make_page(doc_labels[2], "concept_heatmap_network_byDOC_MODERATE_BLACKTEXT_SUB_NOLEG")
make_page(doc_labels[3], "concept_heatmap_network_byDOC_HIGH_BLACKTEXT_SUB_NOLEG")
save_legend_panel("concept_heatmap_network_LEGEND_PANEL_BLACKTEXT")
