...
# Phase normalization + numeric pH titles + PROCESS LEGEND
# X-axis removed (no labels/ticks/title)
...
draw_panel <- function(sub_edges, title_stub) {
  ...
  ggraph::ggraph(lay) +
    ...
    scale_edge_linetype_manual(
      values = c(solid = 1, dashed = 2, dotted = 3),
  ...
  row +
    patchwork::plot_annotation(
      ...
      subtitle = paste0(gsub(" \\(.*?\\)", "", doc_label),
                        " • Columns = pH bands with numeric ranges • Process & mobility legends shown"),
      theme = theme(
...
for (doc in levels(edges$DOC)) {
  for (ctx in levels(edges$context)) {
    ...
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_AXES_PROCESSLEGEND_%s_%s.svg", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H)
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_AXES_PROCESSLEGEND_%s_%s.png", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H, dpi = 600)
  }
}
