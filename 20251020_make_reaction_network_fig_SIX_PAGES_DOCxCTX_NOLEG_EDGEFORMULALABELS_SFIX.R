...
# S-FIX: retain sulfur reactions by filtering nodes by element (not class)
# + sulfur element normalization + dedup phase tags on labels
# ===================================================================
...
# ---- Map mediator to three process classes for line type legend (even though legends are separate) ----
edges <- edges %>% mutate(
...
# Additional sulfur normalization: if element missing or mis-set, infer S from name patterns
...
# ---- Keep only focus elements + the two context anchors ----
...
# ---- Simplify names to formula + phase tag; then dedup any repeated tags ----
...
simplify_formula <- function(s) {
  ...
  repl <- list(
    ...
    # Common sulfur species names -> formulas
...
nodes <- nodes %>%
  mutate(
    label_simple = paste0(simplify_formula(name), " ", phase_tag(phase_norm)) %>%
      trimws() %>%
      # Deduplicate repeated phase tags like "(aq) (aq)" or "(s)(s)"
...
for (doc in levels(edges$DOC)) {
  for (ctx in levels(edges$context)) {
    ...
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_NOLEG_EDGELABELS_SFIX_%s_%s.svg", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H)
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_NOLEG_EDGELABELS_SFIX_%s_%s.png", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H, dpi = 600)
  }
}
