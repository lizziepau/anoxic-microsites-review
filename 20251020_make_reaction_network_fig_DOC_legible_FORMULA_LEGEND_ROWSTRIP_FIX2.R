...
# ===================================================================
# ROWSTRIP (FIXED): proper repelled labels + filtered node draw
# ===================================================================
library(tidyverse)
...
draw_panel <- function(sub_edges, title_stub) {
  ...
  # Data frame for repelled labels
  ...
  p <- ggraph::ggraph(lay) +
    ...
    ggrepel::geom_text_repel(data = lab_df, aes(x = x, y = y, label = label),
                             ...
                             box.padding = 0.16, point.padding = 0.12,
                             max.overlaps = 100, segment.size = 0.12, seed = 123) +
...
for (ctx in ctx_levels) {
  ...
  row <- wrap_plots(c(list(strip), plots), ncol = length(plots) + 1,
                    widths = c(0.05, rep(1, length(plots))), guides = "collect")
...
plate <- wrap_plots(row_plots, ncol = 1, guides = "collect") +
  plot_annotation(
    ...
    subtitle = "Context shown once per row • Formula-only labels • Node shape = mobility state • Legend below",
    theme = theme(
...
ggsave("reaction_network_redox_DOC_legible_FORMULA_LEGEND_ROWSTRIP_FIX2.svg", plate, width = FIG_W, height = FIG_H)
ggsave("reaction_network_redox_DOC_legible_FORMULA_LEGEND_ROWSTRIP_FIX2.png", plate, width = FIG_W, height = FIG_H, dpi = 600)
