...
# ===================================================================
# SIX PAGES (DOC × Context) — NO LEGENDS, EDGE LABELS ABOVE ARROWS
# ===================================================================
...
nodes$element[is.na(nodes$element) & grepl("\\b(SO4|HS|H2S|S\\(0\\)|FeS)\\b", nodes$name)] <- "S"
nodes <- nodes %>% filter((element %in% ELEMENTS_TO_SHOW) | name %in% c("Bulk oxic soil","Anoxic microsite"))
...
nodes <- nodes %>% mutate(
  ...
  phase_norm = case_when(
    ...
    str_detect(phase, regex("bulk", TRUE)) ~ "bulk", TRUE ~ "sorbed")
)
...
nodes <- nodes %>%
  mutate(
    label_simple = paste0(simplify_formula(name), " ", phase_tag(phase_norm)) %>%
      trimws() %>%
      str_replace_all("\\((aq|s|g)\\)\\s*\\1", "(\u005C1)") %>%  # collapse double tags like (aq)(aq)
      str_replace_all("\\s*\\((aq|s|g)\\)\\s*\\((aq|s|g)\\)", "(\\1)") %>%  # catch spaced versions
      str_replace_all("\\s*\\((aq|s|g)\\)\\s*\\1", "(\\1)") %>%  # catch trailing dup
      str_squish()
  )
...
draw_panel <- function(sub_edges, title_stub) {
  ...
  ggraph::ggraph(lay) +
    ...
    scale_y_continuous(breaks = y_map$ypos, labels = y_map$element, expand = expansion(mult = c(0.02, 0.02))) +
...
make_page <- function(edges_doc_ctx, ctx_label, doc_label) {
  ...
  row + patchwork::plot_annotation(
      ...
      subtitle = paste0(gsub(" \\(.*?\\)", "", doc_label),
                        " • Columns = pH bands with numeric ranges • Names shown as reactant → product above arrows"),
...
for (doc in levels(edges$DOC)) {
  for (ctx in levels(edges$context)) {
    ...
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_NOLEG_EDGELABELS_%s_%s.svg", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H)
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_NOLEG_EDGELABELS_%s_%s.png", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H, dpi = 600)
  }
}

# ---- Separate legend figure: process + mobility + mobility state ----
col_mob <- c(mobilize = "#3b5b92", immobilize = "#a35e10", neutral = "#7a7a7a")
proc_types <- c(solid = 1, dashed = 2, dotted = 3)
shape_map <- c("Mobile (aq)" = 21, "Immobile/Sorbed" = 22, "Other" = 25)

df_edges <- expand.grid(mobility_effect = names(col_mob), mediator_class = names(proc_types)) %>%
  mutate(x=0, xend=1, y=row_number(), yend=y)

df_nodes <- tibble::tibble(mobility_state = names(shape_map), x = 0, y = seq_along(mobility_state))

legend_plot <- ggplot() +
  geom_segment(data = df_edges, aes(x=x, y=y, xend=xend, yend=yend, color = mobility_effect, linetype = mediator_class),
  ...
  geom_point(data = df_nodes, aes(x=x, y=y, shape = mobility_state),
             size = 3, stroke = 0.8, show.legend = TRUE, color = "grey20", fill = "white") +
...
ggsave("legend_only_PROCESS_MOBILITY_SHAPE.svg", leg_grob, width = 7, height = 1.2)
ggsave("legend_only_PROCESS_MOBILITY_SHAPE.png", leg_grob, width = 7, height = 1.2, dpi = 600)
