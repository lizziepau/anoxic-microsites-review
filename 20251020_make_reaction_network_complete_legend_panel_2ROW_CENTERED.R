...
# ===================================================================
# COMPLETE LEGEND PANEL — 2 ROWS, CENTERED TITLE
# Exports: legend_panel_COMPLETE_2ROW_CENTERED.svg / .png
# ===================================================================
...
# ---- Arrange into two rows (roughly balanced) ----
n <- length(leg_list)
if (n == 0) {
  stop("No legends found to arrange.")
}
nrow <- 2
ncol <- ceiling(n / nrow)

# If odd, pad with empty grobs to make full grid
if (nrow * ncol > n) {
  pad <- nrow * ncol - n
  leg_list <- c(leg_list, replicate(pad, grid::nullGrob(), simplify = FALSE))
}

grid_leg <- gridExtra::arrangeGrob(grobs = leg_list, nrow = nrow, ncol = ncol,
                                   heights = unit(rep(1, nrow), "null"),
                                   widths  = unit(rep(1, ncol), "null"))

# ---- Wrap with a centered title "Legend" ----
title_g <- textGrob("Legend", gp = gpar(fontface = "bold", cex = 1.05))
legend_panel <- arrangeGrob(
  grobs = list(title_g, grid_leg),
  ncol = 1,
  heights = unit.c(unit(0.22, "in"), unit(1, "null")),
  top = NULL
)

# ---- Save ----
ggsave("legend_panel_COMPLETE_2ROW_CENTERED.svg", legend_panel, width = 7, height = 2.4)
ggsave("legend_panel_COMPLETE_2ROW_CENTERED.png", legend_panel, width = 7, height = 2.4, dpi = 600)
