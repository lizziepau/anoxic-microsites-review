...
# ===================================================================
# THREE DOC LEVELS -> SIX PAGES (per-context) — LEGIBLE (PATCHED)
# Fixes:
#  • Safe legend construction using vertices that match sampled edges
#  • Avoid reserved word "in" in degree calc
#  • Remove `%||%` dependency for label x-nudge
# ===================================================================
library(tidyverse)
...
# --- degree proxy (avoid reserved 'in') ---
deg_tbl <- edges %>%
...
draw_panel <- function(sub_edges, title_stub) {
  ...
  # safe nudge without %||%
  nz <- el_nudge[as.character(lab_df$element)]
  nz[is.na(nz)] <- 0
...
legend_plot <- function() {
  # sample a few edges that definitely exist
  demo_edges <- edges %>% slice_head(n = 6)
  verts_needed <- unique(c(demo_edges$from, demo_edges$to))
  demo_vertices <- nodes %>% filter(name %in% verts_needed)
  if (nrow(demo_vertices) < length(verts_needed)) {
    extra <- setdiff(verts_needed, demo_vertices$name)
    demo_vertices <- bind_rows(demo_vertices, nodes %>% filter(name %in% extra))
  }
  g <- igraph::graph_from_data_frame(d = demo_edges, vertices = demo_vertices, directed = TRUE)
  ...
  # Explicitly grab the first guide-box to avoid the warning
  gb_list <- cowplot::get_plot_component(p, "guide-box", return_all = TRUE)
  if (is.list(gb_list)) gb <- gb_list[[1]] else gb <- gb_list
  gb
}
