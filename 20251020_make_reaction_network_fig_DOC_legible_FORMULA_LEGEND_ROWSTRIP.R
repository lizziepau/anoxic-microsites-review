...
source("make_reaction_network_fig_DOC_legible_FORMULA_LEGEND_ROWSTRIP.R")
# -> reaction_network_redox_DOC_legible_FORMULA_LEGEND_ROWSTRIP.svg/.png

# ===================================================================
# Legible (FORMULA + LEGEND) — Context strip at row-left, anti-overlap labels,
# Mobility-state symbols, legend below.
# ===================================================================
library(tidyverse)
...
# Filter to requested elements (keep default for infrastructure)
...
# Mobility-state symbol (node shape) inferred from phase
nodes <- nodes %>% mutate(
...
# Label thinning (no environment labels)
...
# -------------- draw one small panel (no context in title) --------------
...
# Build keys: DOC × pH × context
edges <- edges %>% mutate(
  key = interaction(DOC, pH_band, context, sep = " • "),
  title_stub = interaction(DOC, pH_band, sep = " • ")
)

# Order: contexts as rows; within each, DOC × pH
...
for (ctx in ctx_levels) {
  ...
  for (i in seq_along(ord_keys)) {
    ...
    title_txt <- gsub(" \\(.*?\\)", "", k)  # strip parentheses to save space
    plots[[i]] <- draw_panel(sub_edges, title_stub = title_txt)
  }
  # left-side vertical context strip
  strip <- ggplot() +
    annotate("text", x = 0, y = 0.5, label = ctx, angle = 90, vjust = 0.5, hjust = 0.5,
             size = BASE_TEXT/3, fontface = "bold") +
    theme_void() +
    theme(plot.margin = margin(0,0,0,0))
  # assemble: strip (ncol=1) + row of panels (ncol=length(ord_keys))
  row <- wrap_plots(c(list(strip), plots), ncol = length(plots) + 1, widths = c(0.04, rep(1, length(plots))),
                    guides = "collect")
  row_plots[[length(row_plots) + 1]] <- row
}

plate <- wrap_plots(row_plots, ncol = 1) +
  plot_annotation(
...
ggsave("reaction_network_redox_DOC_legible_FORMULA_LEGEND_ROWSTRIP.svg", plate, width = FIG_W, height = FIG_H)
ggsave("reaction_network_redox_DOC_legible_FORMULA_LEGEND_ROWSTRIP.png", plate, width = FIG_W, height = FIG_H, dpi = 600)
