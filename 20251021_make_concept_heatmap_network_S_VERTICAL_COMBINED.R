...
# File: make_concept_heatmap_network_S_VERTICAL_COMBINED.R
# Updates:
# - DOC facet labels include numerical ranges with scientific style:
#   Low DOC (< 2 mg C L⁻¹), Moderate DOC (2–10 mg C L⁻¹), High DOC (> 10 mg C L⁻¹)
# - pH labels remain split across two lines
# - Process legend uses the character "o" for keys (colored by process)
# ===================================================================
...
# ---- Titles ----
PANEL_TITLE <- "Reaction network in oxic soil with anoxic microsites — combined conceptual heatmap"
SUBTITLE_TXT <- "Columns = DOC (with numeric ranges); within each DOC: x = pH band, y = element; tile fill = net mobility; arrow color = process"
...
p <- ggplot(cell, aes(x = pH_band, y = element)) +
  ...
  # Process legend shows "o" for the text geom
  scale_color_manual(values = proc_col, name = "Process",
                     guide = guide_legend(override.aes = list(label = "o"))) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 8.8, lineheight = 0.95, vjust = 1, hjust = 0.5),
...
# ---- Save ----
ggsave("concept_heatmap_network_S_VERTICAL_COMBINED.svg", p, width = FIG_W, height = FIG_H)
ggsave("concept_heatmap_network_S_VERTICAL_COMBINED.png", p, width = FIG_W, height = FIG_H, dpi = 600)
