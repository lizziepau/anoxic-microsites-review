...
# ===================================================================
# SIX PAGES (DOC × Context) — MINIMAL & LEGIBLE with PHASE NORMALIZATION + NUMERIC pH TITLES
...
nodes$element <- nodes$element %>%
  replace_na("default") %>%
  recode("Vanadium"="V", "V(V)"="V", .default = .)
...
for (doc in levels(edges$DOC)) {
  for (ctx in levels(edges$context)) {
    ...
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_%s_%s.svg", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H)
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_%s_%s.png", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H, dpi = 600)
  }
}
