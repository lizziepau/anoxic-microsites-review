...
# ===================================================================
# Fitting-focused network plot (PATCH v2): manual pagination w/ patchwork
# ===================================================================
library(tidyverse)
...
draw_panel <- function(sub_edges, panel_title) {
  ...
  p <- ggraph::ggraph(lay) +
    ...
    ggraph::geom_node_point(aes(shape = class, fill = phase, color = element),
                            size = 2.7, stroke = 0.9, show.legend = FALSE) +
    ...
    ggraph::geom_node_text(aes(label = label),
                           size = LABEL_SIZE, vjust = -0.9, lineheight = 0.98) +
...
for (pg in seq_len(pages)) {
  ...
  ggsave(sprintf("reaction_network_fitting_page%02d.svg", pg), pgrid, width = FIG_W, height = FIG_H)
  ggsave(sprintf("reaction_network_fitting_page%02d.png", pg), pgrid, width = FIG_W, height = FIG_H, dpi = 600)
}

message("Saved ", pages, " pages.")
