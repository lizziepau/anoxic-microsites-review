...
# ===================================================================
# SIX PAGES (DOC × Context) — S ARROWS FIX via NAME NORMALIZATION
# Keeps sulfur reactions, matches edges<->nodes using normalized keys
# ===================================================================
...
norm_key <- function(x) {
  x %>%
    # Normalize Unicode to ASCII-ish tokens
    ...
    str_replace_all("−","-") %>% str_replace_all("·","·") %>%  # keep middot but unify minus
    str_replace_all("–","-") %>% str_replace_all("—","-") %>%
    str_replace_all("’","'") %>%
    # Common chemistry tokens
    str_replace_all("SO₄","SO4") %>% str_replace_all("H₂S","H2S") %>%
    # Remove spaces around parentheses and collapse multiple spaces
...
edges <- edges %>% left_join(map_from, by = c("from_key" = "key")) %>%
  left_join(map_to,   by = c("to_key"   = "key")) %>%
  mutate(from = coalesce(from_name, from),
         to   = coalesce(to_name,   to)) %>%
...
lane_map <- c("solid/colloid"=0.08, "sorbed"=0.16, "aq"=0.50, "microsite"=0.98, "bulk"=0.02)
default_lane <- 0.50
# (Recompute after any node filtering just in case)
nodes <- nodes %>% mutate(lane = ifelse(phase_norm %in% names(lane_map), lane_map[phase_norm], default_lane))
nodes <- nodes %>% mutate(lane = case_when(name=="Bulk oxic soil" ~ 0.02, name=="Anoxic microsite" ~ 0.98, TRUE ~ lane))
...
for (doc in levels(edges$DOC)) {
  for (ctx in levels(edges$context)) {
    ...
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_NOLEG_EDGELABELS_SFIX_KEY_%s_%s.svg", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H)
    ggsave(sprintf("reaction_network_MIN_NORM_PHT_NOLEG_EDGELABELS_SFIX_KEY_%s_%s.png", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H, dpi = 600)
  }
}
