...
# ===================================================================
# Three-panel plate by DOC level (7.0" x 4.167")
# Fixed at: pH = circumneutral (6.0–7.5), Context = Anoxic microsite
# Panels:
#   1) Low DOC (<2 mg C/L)
#   2) Moderate DOC (2–10 mg C/L)
#   3) High DOC (>10 mg C/L)
# ===================================================================
library(tidyverse)
...
# ------- Facets: one per DOC (fixed pH + context) -------
FACETS_TO_INCLUDE <- c(
  "Low DOC (<2 mg C/L) • circumneutral (6.0–7.5) • Anoxic microsite",
...
plate <- wrap_plots(plots, ncol = NCOL, byrow = TRUE, guides = "collect") +
  plot_annotation(
    ...
    subtitle = "Three panels by DOC level (pH 6.0–7.5, anoxic microsites) • Formula-only labels • Focus elements labeled",
    theme = theme(
...
ggsave("reaction_network_THREE_PANEL_byDOC.svg", plate, width = FIG_W, height = FIG_H)
ggsave("reaction_network_THREE_PANEL_byDOC.png", plate, width = FIG_W, height = FIG_H, dpi = 600)
