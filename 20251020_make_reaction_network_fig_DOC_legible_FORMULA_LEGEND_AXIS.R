...
# ===================================================================
# Legible (FORMULA + LEGEND) — Context on Y-axis, extra label spacing
# ===================================================================
library(tidyverse)
...
# tighter canvas text; nudge labels up a bit
LABEL_SIZE <- 3.0
BASE_TEXT <- 11
LABEL_NUDGE_Y <- 0.28   # vertical offset to clear the symbols
POINT_SIZE <- 3.0       # slightly smaller to reduce collisions
...
# label thinning — now EXCLUDES environment so they won't be drawn
pick_label_nodes <- function(layout_df, focus_elements, label_sorbents = FALSE, label_ligands = FALSE) {
  ...
  keep <- (d$class == "species" & is_focus) # environment removed from labels
...
draw_panel <- function(sub_edges, panel_title) {
  # y-axis label = context (unique per panel)
  y_lab <- unique(as.character(sub_edges$context))
  ...
  # draw nodes except environment
  non_env <- layout_df$class != "environment"
  ...
  ggraph::ggraph(lay) +
    ...
    ggraph::geom_node_point(aes(shape = class, fill = phase, color = element),
                            size = POINT_SIZE, stroke = 1, show.legend = TRUE,
                            data = as.data.frame(lay)[non_env, ]) +
    ...
    ggraph::geom_node_text(aes(label = ifelse(label_mask, label, "")),
                           size = LABEL_SIZE, vjust = -1.0, nudge_y = LABEL_NUDGE_Y,
                           lineheight = 0.98, check_overlap = TRUE) +
    ...
    theme(
      ...
      axis.title.y = element_text(size = BASE_TEXT, face = "bold"),
      axis.text.y = element_blank(),
      axis.ticks.y = element_blank()
    ) +
    labs(title = panel_title, x = NULL, y = y_lab)
}
...
plate <- wrap_plots(plots, ncol = NCOL, byrow = TRUE, guides = "collect") +
  plot_annotation(
    ...
    subtitle = "Context moved to Y-axis • Labels nudged to avoid symbol overlap • Legends collected",
    theme = theme(
...
ggsave("reaction_network_redox_DOC_legible_FORMULA_LEGEND_AXIS.svg", plate, width = FIG_W, height = FIG_H)
ggsave("reaction_network_redox_DOC_legible_FORMULA_LEGEND_AXIS.png", plate, width = FIG_W, height = FIG_H, dpi = 600)
