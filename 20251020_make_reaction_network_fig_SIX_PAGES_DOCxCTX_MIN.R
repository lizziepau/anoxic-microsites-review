...
source("make_reaction_network_fig_SIX_PAGES_DOCxCTX_MIN.R")
...
suppressPackageStartupMessages({
  library(ggraph); library(igraph); library(patchwork); library(ggrepel)
})
...
MAX_LABELS_PER_ELEMENT <- 1         # <— fewer labels per element
LABEL_SIZE <- 2.1
BASE_TEXT <- 8.2
...
# species-only to reduce clutter
...
# lanes
lane_map <- c("solid"=0.05, "sorbed"=0.12, "aq/adsorbed"=0.18,
...
nodes <- nodes %>% mutate(
  mobility_state = case_when(
    str_detect(phase, "aq") ~ "Mobile (aq)",
    str_detect(phase, "sorb|solid|colloid|oxide") ~ "Immobile/Sorbed",
    TRUE ~ "Other"
  )
)
...
# Degree proxy (avoid reserved word "in")
deg_tbl <- edges %>%
...
# Phase ranking — prefer aqueous labels
...
pick_label_nodes <- function(layout_df) {
  df <- layout_df %>%
    ...
    # label whitelist: only aqueous-like phases
    filter(phase %in% c("aq","aq/adsorbed","aq/sorbed"))
  df <- df %>%
    group_by(element) %>%
...
draw_panel <- function(sub_edges, title_stub) {
  ...
  ggraph::ggraph(lay) +
    ...
    scale_edge_linetype_manual(values = lty_mediator, guide = "none") +   # drop mediator legend
    ...
    scale_fill_manual(values = fill_map_phase, guide = "none") +          # drop phase legend
    scale_color_manual(values = stroke_map, guide = "none") +             # drop element legend
...
make_page <- function(edges_doc_ctx, ctx_label, doc_label) {
  ...
  plots <- lapply(ph_levels, function(k) {
    ...
    # keep numeric pH values for clarity
    title_txt <- case_when(
      grepl("acidic", k)        ~ "Acidic (pH 4.5–6.0)",
      grepl("circumneutral", k) ~ "Circumneutral (pH 6.0–7.5)",
      grepl("alkaline", k)      ~ "Alkaline (pH 7.5–8.5)",
      TRUE                      ~ k
    )
  ...
  row +
    plot_annotation(
      ...
      subtitle = paste0(gsub(" \\(.*?\\)", "", doc_label),
                        " • Columns = pH (acidic, circumneutral, alkaline) • Legends: mobility only"),
      theme = theme(
...
for (doc in levels(edges$DOC)) {
  for (ctx in levels(edges$context)) {
    ...
    ggsave(sprintf("reaction_network_MIN_byDOC_%s_%s.svg", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H)
    ggsave(sprintf("reaction_network_MIN_byDOC_%s_%s.png", stub_doc, stub_ctx), plate, width = FIG_W, height = FIG_H, dpi = 600)
  }
}

unique(nodes$element)
