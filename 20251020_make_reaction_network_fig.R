...
# =========================
# Reaction network: Oxic soil with anoxic microsites (pH 4.5–8.5)
# =========================
library(tidyverse)
...
edges <- read_csv("redox_network_edges.csv", show_col_types = FALSE)
...
lty_mediator <- c(
  ...
  "microbial/Fe(II)"=3, "Mn/Fe oxides"=2, "O₂/NO₃⁻"=2
)
...
p <- ggraph(g, layout = "fr") +
  geom_edge_link(aes(color = mobility_effect, linetype = lty),
                 arrow = arrow(length = unit(2.5, "mm")),
                 end_cap = circle(2.2, 'mm'),
                 edge_width = 0.5, show.legend = TRUE) +
  ...
  labs(
    title = "Reaction network for metals in oxic soils with anoxic microsites (pH 4.5–8.5)",
    subtitle = "Nodes: species/sorbents/ligands • Edges: transformations • Edge color = mobilization vs immobilization",
    x = NULL, y = NULL
  )

ggsave("reaction_network_redox.svg", p, width = 12.0, height = 9.5)
ggsave("reaction_network_redox.png", p, width = 12.0, height = 9.5, dpi = 600)
